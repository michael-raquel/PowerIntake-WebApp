...
export default function TicketWorkspace({ routeRole = null }) {
  ...
  const roleViewIds = role === "super-admin"
    ? ["my-client", "my-company", "my-team", "my-ticket"]
    : role === "admin"
      ? ["my-company", "my-team", "my-ticket"]
      : role === "manager"
        ? ["my-team", "my-ticket"]
        : ["my-ticket"];
  ...
  const scopedTickets = useMemo(() => {
    ...
    return allTickets.filter((ticket) => normalizeValue(getTicketValue(ticket, "ownerId")) === normalizeValue(currentUserId));
  ...
  const filterOptions = (() => {
    ...
    for (const filter of viewConfig?.filters ?? []) {
      ...
      options[filter] = [...new Set(scopedTickets.map((ticket) => getTicketValue(ticket, field)).map((v) => String(v ?? "").trim()).filter(Boolean))];
    }
    return options;
  })();
  ...
  const filteredTickets = scopedTickets.filter((ticket) => {
    const searchOk =
      !query ||
      (viewConfig?.searchableFields ?? []).some((field) => normalizeValue(getTicketValue(ticket, field)).includes(query));
  ...
  return (
    <div className="flex-1 bg-gray-50 dark:bg-black min-h-screen transition-colors duration-300">
        ...
        {totalRecords > 0 && (
          <div className="bg-white dark:bg-gray-900 px-4 py-3 rounded-lg border border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="text-sm text-gray-700 dark:text-gray-300 order-2 sm:order-1">
              Showing <span className="font-medium text-gray-900 dark:text-white">{paginationInfo?.start}</span> to{" "}
      ...
      {selectedTicket && <ComUpdateForm ticket={selectedTicket} onClose={() => setSelectedTicket(null)} />}
    </div>
  );
}
