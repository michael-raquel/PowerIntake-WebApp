...
const COLUMNS = [
  'Ticket ID', 'Client', 'Manager', 'User',
  'Title', 'Category', 'Priority', 'Created',
  'Status', 'Days', 'Technician',
];
            ...
            {paginatedData.length > 0 ? (
              paginatedData.map((ticket) => (
                <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors duration-150">
                  ...
                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 max-w-[100px] truncate" title={ticket.clientName}>{ticket.clientName}</td>
                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 max-w-[80px] truncate" title={ticket.managerName}>{ticket.managerName}</td>
