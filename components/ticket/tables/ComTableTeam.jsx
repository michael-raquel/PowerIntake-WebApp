...
export default function ComTableTeam({
  ...
  const { data: teamTickets, loading, error, setData: setTeamTickets } = useFetchMyTeamUsers({
  ...
  useEffect(() => {
    ...
    const handleTicketDeleted = ({ ticketuuid }) => {
      setTeamTickets(prev => prev.filter(t => t.v_ticketuuid !== ticketuuid));
  ...
  }, [setTeamTickets, onSynced, onSyncFailed, onDeleted, onUpdated, selectedTicket]);
  ...
  useEffect(() => {
    ...
    if (ticketsChanged) {
      ...
      for (const [filterName, { key, label }] of Object.entries(FIELD_LABELS)) {
        const values = teamTickets.map(t => t[key]);
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-800">
              {['TICKET ID', 'SOURCE', 'USER NAME', 'TITLE', 'CATEGORY', 'PRIORITY', 'CREATED AT', 'TARGET', 'STATUS', 'TECHNICIAN'].map(header => (
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {paginated.length === 0 ? (
              <tr>
                <td colSpan={10} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No tickets found.</td>
              </tr>
            ) : (
              paginated.map(t => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300 max-w-[200px] truncate">{t.v_title || '—'}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300 whitespace-nowrap">{t.v_ticketcategory || '—'}</td>
