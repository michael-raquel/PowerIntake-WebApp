...
export default function ComAttachment({ attachments = [], onChange }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {!attachments.length ? (
        <div className="flex flex-col items-center justify-center py-8 text-gray-400 text-sm border border-dashed border-gray-300 rounded-lg">
        ...
        <div className="space-y-2">
          {attachments.map((a, index) => (
            <div key={`${a.name}-${index}`} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border">
              <FileText className="w-4 h-4 text-blue-500 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-800 truncate">{a.name}</p>
                {(a.createdAt || a.v_createdat) && (
                  <p className="text-xs text-gray-400">{format(new Date(a.createdAt || a.v_createdat), 'MMM dd, yyyy')}</p>
                )}
              </div>
              <Button variant="ghost" size="icon" onClick={() => handleRemove(index)} className="h-7 w-7 shrink-0">
