...
export default function SpartaAssistWidget() {
  ...
  const frameHeightBase = isThinPhone ? 390 : isMobile ? 460 : isLarge ? 500 : 415;
  const frameHeight =
    viewport.h > 0
  ...
  return (
    <>
      <button
        ...
        style={{
          position: "fixed",
          right: `max(${edgeGap}px, env(safe-area-inset-right))`,
          bottom: `max(${edgeGap}px, env(safe-area-inset-bottom))`,
          width: launcherSize,
          height: launcherSize,
          borderRadius: "50%",
      ...
      <div
        ...
        style={{
          position: "fixed",
          right: `max(${edgeGap}px, env(safe-area-inset-right))`,
          bottom: `calc(max(${edgeGap}px, env(safe-area-inset-bottom)) + ${launcherSize + 10}px)`,
          width: frameWidth,
          height: frameHeight,
          maxWidth: `calc(100vw - ${edgeGap * 2}px)`,
          maxHeight: `calc(100vh - ${launcherSize + edgeGap + 24}px)`,
